# -*- coding: utf-8 -*-
"""
Created on Sat Dec 24 11:13:01 2016
...
class baxter_kinematics(object):
    ...
    def __init__(self, limb):
        ...
        self._kdl_tree = kdl_tree_from_urdf_model(self._baxter)     #from Robot
        ...
        #Vector Zero !! Need to be changed for considering the mass effect
    ...
    def print_robot_description(self):
        ...
        for j in self._baxter.joints:
            if j.type != 'fixed':
                nf_joints += 1      #non-fixed joints counter
    ...
    def print_kdl_chain(self):
        for idx in xrange(self._arm_chain.getNrOfSgments()):
            print '*' + self._arm_chain.getSegment(idx).getName()


    #Gain the interface info to KDL info
    ...
    def forward_position_kinematics(self):
        ...
        pos = end_frame.p
        rot = PyKDL.Rotation(end_frame.M)   #(pos, R)
        rot = rot.GetQuaternion()
        return np.array([pos[0], pos[1], pos[2], 
                         rot[0], rot[1], rot[2], rot[3]])
    
    def forward_velocity_kinematics(self):
        end_frame = PyKDL.FrameVel()        # C++- Specific
        self._fk_v_kdl.JntToCart(self.joints_to_kdl('velocities'), end_frame)  #Gain from the interface
        return end_frame.GetTwist()     #Get 6D vector
    ...
    #The total inertia Matrix in the Jnt Space:
    def inertia(self):
        inertia = PyKDL.JntSpaceInertiaMatrix(self._num_jnts)
        self._dyn_kdl.JntToMass(self.joints_to_kdl('posiitons'), inertia)
        return self.kdl_to_mat(inertia)

    #The total inertia Matrix in the Cart Space
